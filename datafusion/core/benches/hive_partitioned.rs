...
fn run_benchmark(c: &mut Criterion) {
    ...
    rt.block_on(async {
        ...
        for (name, query) in queries.iter() {
            // Optimized
            let config_opt = SessionConfig::new().with_target_partitions(16).set_bool(
            ...
            // Unoptimized
            let config_unopt = SessionConfig::new().with_target_partitions(16).set_bool(
                "datafusion.execution.listing_table_preserve_partition_values",
                false,
            );
    ...
    group.bench_function("preserve_partition_values=true", |b| {
        b.to_async(&rt).iter(|| async {
            let config = SessionConfig::new()
                .with_target_partitions(16) // Ensure parallelism
                .set_bool(
                    "datafusion.execution.listing_table_preserve_partition_values",
                    true,
                );
    ...
    group.bench_function("preserve_partition_values=false", |b| {
        b.to_async(&rt).iter(|| async {
            let config = SessionConfig::new().with_target_partitions(16).set_bool(
                "datafusion.execution.listing_table_preserve_partition_values",
                false,
            );
            let ctx = SessionContext::new_with_config(config);
